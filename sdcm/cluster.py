...
class BaseMonitorSet:  # pylint: disable=too-many-public-methods,too-many-instance-attributes
    ...
    def configure_overview_template(self, node: BaseNode):
        ...
        template_file = f"/tmp/scylla-overview.template.{node.name}.json"
        node.remoter.receive_files(src=str((self.monitor_install_path / "grafana" /
                                   "scylla-overview.template.json").absolute()), dst=template_file)
        ...
        with open(template_file, "wt") as file:
            json.dump(template, file, indent=4)

        node.remoter.send_files(src=template_file, dst=str((self.monitor_install_path /
                                "grafana" / "scylla-overview.template.json").absolute()))
